...
gld <- read.csv("~/tmp/gld/gmp/78g1/run_2015-july-1st-31st_xfo-to-load-direct/1/underground_line/current_out/60071.csv", header = TRUE) %>%
	mutate(local_time = force_tz(ymd_hms(local_time, tz = 'America/New_York')))

scada <- read.csv("~/data/proc/gmp/78g1/scada/2015-july-1st-31st.csv", header = TRUE) %>%
	mutate(local_time = force_tz(ymd_hms(local_time, tz = 'America/New_York')))
...
ggplot() +
	...
	ggtitle(paste("Mean Error (iDGA minus SCADA) = ", sprintf("%.1f", mean(y_gld.3phase_ave - y_scada.3phase_ave, na.rm = TRUE)), sep = "")) +
	xlab("Time") +
...
df <- read.csv('~/tmp/gld/gmp/78g1/inp_cp__{device_number}__{phase}.csv',
	       colClasses = c('character', 'complex'),
	       header     = FALSE) %>%
