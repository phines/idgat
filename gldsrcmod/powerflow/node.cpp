		...
		if (NR_busdata==NULL || NR_branchdata==NULL)	//First time any NR in (this should be the swing bus doing this)
		{
			...
			if (NRMatDumpMethod != MD_NONE) {
				if (filename_md[0] == '\0') {
					...
					/*
					  TROUBLESHOOTING:
					  The powerflow module was configured such that a Newton-Raphson-based matrix dump was desired.
					  However, no file was specified in NR_matrix_file.  No output will be performed.
					*/
					NRMatDumpMethod = MD_NONE;
				}
				else {
					/* pracherla9 (10/17/2106) --
					  NRFileDump = fopen(filename_md,"wt");
